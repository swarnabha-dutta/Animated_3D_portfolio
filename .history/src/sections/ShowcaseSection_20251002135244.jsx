...
const AppShowcase = () => {
  ...
  useGSAP(() => {
    // Optimize animation - only animate when visible
    gsap.fromTo(
      sectionRef.current,
      { opacity: 0 },
      { 
    ...
    const cards = [
      doctorAppointmentProjectRef.current, 
      codeReviewerRef.current, 
      groupChatWithAIRef.current
    ];
    ...
    cards.forEach((card, index) => {
      ...
      gsap.fromTo(
        card,
        { y: 30, opacity: 0 }, // Reduced from 50
        {
          y: 0,
          opacity: 1,
          duration: 0.8, // Reduced from 1s
          delay: 0.2 * (index + 1), // Reduced from 0.3
          ease: "power2.out",
          scrollTrigger: {
            trigger: card,
        ...
        <div className="showcaselayout">
          <div ref={doctorAppointmentProjectRef} className="first-project-wrapper">
            <div className="image-wrapper">
              <OptimizedImage 
                src="/images/HealthCare_platform.jpg" 
                alt="HealthCheckBuddy App Interface" 
              />
            </div>
